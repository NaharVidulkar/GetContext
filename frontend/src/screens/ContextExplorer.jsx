...
export default function ContextExplorer({ repo, onAsked }) {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
      ...
      <h2 style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: 26, color: '#FFFFFF', margin: '0 0 8px', letterSpacing: '-0.03em' }}>Context Explorer</h2>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <div style={{
            ...
            <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 14, color: 'var(--accent-light)', marginRight: 12 }}>&gt;_</span>
            <input
              type="text"
              value={query}
              ...
              style={{ flex: 1, backgroundColor: 'transparent', border: 'none', color: '#FFFFFF', fontFamily: 'Inter, sans-serif', fontSize: 15, padding: '10px 0', outline: 'none' }}
            />
          </div>
          <PrimaryButton onClick={() => handleAsk()} disabled={loading} style={{ flexShrink: 0, height: 48, borderRadius: 14, padding: '0 28px' }}>
            {loading ? '…' : 'Ask →'}
          </PrimaryButton>
        </div>
      ...
      {loading && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {[1, 2, 3].map((i) => (
            <div key={i} style={{ backgroundColor: 'rgba(18, 21, 30, 0.85)', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: 14, padding: 24, opacity: 1 - i * 0.15 }}>
      ...
      {result && !loading && (
        <>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: 'var(--muted)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {result.relevantFiles.length > 0 && (
            <div style={{ marginTop: 24, display: 'flex', justifyContent: 'flex-end' }}>
              <PrimaryButton onClick={() => onAsked(result)}>Generate AI Response →</PrimaryButton>
            </div>
          )}
        </>
      )}
