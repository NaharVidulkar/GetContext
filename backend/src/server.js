...
import { analyzeRepository, fetchRecentCommits } from './githubFetcher.js';
...
async function buildAndSaveRecord(url, analysis) {
  ...
  // Full-repo context export needs actual snippets of the key files, not
  // just their paths — pull short previews of the top-ranked files so the
  // frontend can build a "understand this whole codebase" block without a
  // second round-trip.
  const KEY_FILE_PREVIEW_CHARS = 1500;
  const keyFiles = (analysis.importantFiles || []).map((path) => {
  ...
  const record = {
    ...
    keyFiles, // NEW: [{ path, preview }] for the whole-repo context export
    recentCommits: analysis.recentCommits || [], // "what was just being worked on"
    files: analysis.files, // used for retrieval only, stripped before response
    repositoryTokens,
    indexed: true,
...
app.listen(PORT, () => {
  console.log(`GetContext backend running on http://localhost:${PORT}`);
