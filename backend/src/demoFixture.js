...
export function getDemoFixture() {
  ...
  return {
    owner: 'getcontext-demo',
    repo: 'taskflow',
    fullName: 'getcontext-demo/taskflow',
    ...
    recentCommits: [
      { message: 'WIP: add rate limiter to auth routes, not tested yet', author: 'alex', date: new Date(Date.now() - 3 * 3600e3).toISOString() },
      { message: 'Fix JWT expiry check in authMiddleware', author: 'alex', date: new Date(Date.now() - 26 * 3600e3).toISOString() },
      { message: 'Add task CRUD routes', author: 'sam', date: new Date(Date.now() - 50 * 3600e3).toISOString() },
      { message: 'Initial commit: project scaffold', author: 'sam', date: new Date(Date.now() - 96 * 3600e3).toISOString() },
    ],
    isDemoFixture: true,
  };
}
