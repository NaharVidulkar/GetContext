...
async function githubGet(url) {
  const headers = { 'User-Agent': 'getcontext-hackathon-mvp', Accept: 'application/vnd.github+json' };
...
export async function analyzeRepository(url) {
  ...
  for (const f of toFetch) {
    ...
    try {
      ...
      const res = await fetch(rawUrl, { headers: { 'User-Agent': 'getcontext-hackathon-mvp' } });
      if (!res.ok) continue;
...
// Recent commits = a cheap, automatic trail of "what was just being worked
// on" — the missing half of "resume work in a new agent." No LLM needed,
// just GitHub's own history.
export async function fetchRecentCommits(owner, repo, defaultBranch, limit = 8) {
  try {
    const commits = await githubGet(
      `https://api.github.com/repos/${owner}/${repo}/commits?sha=${defaultBranch}&per_page=${limit}`
    );
    ...
    return []; // never fail analysis just because commit history is unavailable
  }
}
